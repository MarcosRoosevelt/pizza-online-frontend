...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (pizzasSelecionadas.length === 0) {
      ...
      for (const flavorId of flavorIds) {
        try {
          const res = await axios.get(
            `https://pizzaonline-cdbnfabqbgdcasa9.brazilsouth-01.azurewebsites.net/flavors/${flavorId}`,
            {
              headers: {
          ...
          console.error(
            `Erro ao obter detalhes do sabor com ID ${flavorId}:`,
            error
          );
        }
      }
    ...
    const pizzasJSON = await Promise.all(
      pizzasSelecionadas.map(async (pizzaData) => {
        const pizza = pizzaData.pizza;
        const flavorIds = pizza.flavorIds || [];
        const flavors = [];
        for (const flavorId of flavorIds) {
    ...
    concluirPedidoBtn.addEventListener("click", async function () {
      try {
        const res = await axios.post(
          "https://pizzaonline-cdbnfabqbgdcasa9.brazilsouth-01.azurewebsites.net/orders",
          {
            pizzas: pizzasJSON,
          },
          {
...
async function getUser(token) {
  try {
    const res = await axios.get(
      "https://pizzaonline-cdbnfabqbgdcasa9.brazilsouth-01.azurewebsites.net/clientes",
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
