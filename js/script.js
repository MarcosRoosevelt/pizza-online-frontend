  ...
  button.addEventListener("click", async function (e) {
    ...
    try {
      const response = await axios.get(
        "https://pizzaonline-cdbnfabqbgdcasa9.brazilsouth-01.azurewebsites.net/flavors"
      );
      const flavors = response.data;

      const flavorButtons = flavors
        .map(
          (flavor) => `
      ...
      const finalizeButton =
        '<button class="finalize">Finalizar Pedido</button>';
      ...
      document
        .querySelector(".finalize")
        .addEventListener("click", async function () {
          const usuarioAutenticado = verificarAutenticacao();

          if (!usuarioAutenticado) {
            alert(
              "Você precisa estar logado para finalizar o pedido. Por favor, faça o login."
            );
            return;
          }
          ...
          document.querySelectorAll(".flavor").forEach((flavor) => {
            const quantity = parseInt(
              flavor.querySelector(".quantity").textContent
            );
            if (quantity > 0) {
